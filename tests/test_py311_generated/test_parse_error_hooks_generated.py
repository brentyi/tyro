...
def test_missing_subcommand() -> None:
    ...
    _run_expecting_exit(seen.append, CommandA | CommandB, [])
    (event,) = seen
...
def test_mutex_conflict() -> None:
    ...
    def main(
        option_a: Annotated[str | None, Group] = None,
        option_b: Annotated[int | None, Group] = None,
    ) -> None:
        del option_a, option_b
...
def test_missing_args_in_subcommand() -> None:
    ...
    with _capture(hook):
        with pytest.raises(SystemExit):
            tyro.cli(CommandA | CommandB, args=["command-b", "--flag"])
    (event,) = seen
