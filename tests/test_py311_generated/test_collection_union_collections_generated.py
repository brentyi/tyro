...
def test_list_union_with_str_and_tuple() -> None:
    ...
    def main(x: List[str | Tuple[int, int]]) -> List[str | Tuple[int, int]]:
        return x
...
def test_list_union_triple_tuple_sizes() -> None:
    ...
    def main(
        x: List[Tuple[int] | Tuple[int, int] | Tuple[int, int, int]],
...
def test_nested_list_union() -> None:
    def main(x: List[List[int | Tuple[int, str]]]) -> Any:
        return x
...
def test_nested_union_in_collection() -> None:
    ...
    # Works because exact multiples.
    result = tyro.cli(
        List[Tuple[int, int] | Tuple[int, int, int] | bool],
        args=["1", "2", "3", "4"],
    )
    assert result == [(1, 2), (3, 4)]
