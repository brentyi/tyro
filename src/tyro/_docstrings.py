...
def get_class_tokenization_with_field(
    ...
    if dataclasses.is_dataclass(cls) and found_source and not found_field:
        ...
        warnings.warn(
            f"Field '{field_name}' not found in tokenized source for {cls.__name__}. "
            "This may indicate multiple dataclasses with the same name in different scopes, "
            "a dynamically generated field, or a source parsing issue.",
            stacklevel=2,
        )
